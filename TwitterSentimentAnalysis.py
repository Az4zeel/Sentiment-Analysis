...
def getPositiveTweets(text):
    positives=getEmotions(text,clf_positive)
    return("Positive tweet ->%s"%(text))

def getNegativeTweets(text):
    negatives=getEmotions(text,clf_negative)
    return("Negative tweet ->%s"%(text))

def getNeutralTweets(text):
    positives=getEmotions(text,clf_positive)
    negatives=getEmotions(text,clf_negative)
    if(positives and negatives) <1:
        return("Neutral tweet ->%s"%(text))

train_data_csv_name="Trump_Train_data.csv"
...
tweet_file_name="Trump_Test_data.csv"
with open(tweet_file_name, encoding="utf-8", errors = 'ignore') as f:
    for line in f:
        try:
            ...
            pass       
outputfile.close()
outputfile = open('positivetweets.txt','w')
tweet_file_name="Trump_Test_data.csv"
with open(tweet_file_name, encoding="utf-8", errors = 'ignore') as f:
    for line in f:
        try:
            tmpstr=getPositiveTweets(line)
            print(tmpstr)
            outputfile.write(tmpstr)
        except:
            pass       
outputfile.close()
outputfile = open('neutraltweets.txt','w')
tweet_file_name="Trump_Test_data.csv"
with open(tweet_file_name, encoding="utf-8", errors = 'ignore') as f:
    for line in f:
        try:
            tmpstr=getNeutralTweets(line)
            print(tmpstr)
            outputfile.write(tmpstr)
        except:
            pass       
outputfile.close()
outputfile = open('negativetweets.txt','w')
tweet_file_name="Trump_Test_data.csv"
with open(tweet_file_name, encoding="utf-8", errors = 'ignore') as f:
    for line in f:
        try:
            tmpstr=getNegativeTweets(line)
            print(tmpstr)
            outputfile.write(tmpstr)
        except:
            pass       
